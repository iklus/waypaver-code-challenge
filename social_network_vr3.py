...
class SocialNetwork(object):
    def __init__(self, word, wordList):
        ...
        self.network = set([])  # Stores all of the words in the social 
                                    #   network starting with the word.
        self.friends = set([])   # Stores all the friends.
        self.friendsOf2 = set([])    # Stores all the friends of friends.
        self.friendsOf3 = set([])    # Stores all the friends of friends of friends.

    """Takes in a set of words and finds all of the friends for each word in
    the set and returns a set comprised of all of the found friends.
    """
        ...
        for line in toBeSearched:
            ...
            if length in self.wordList:
                for word in self.wordList[length]:
                    diffLetters = 0
                    # If two or more letters are different, the words aren't
                    #   friends, and it breaks the for loop. 
    ...
    def findNetwork(self):
        ...
        self.friends = self.findFriends(set([self.word]))
        
        self.network += self.friends
        
        # Finds friends of friends and removes previous friends from list.
        self.friendsOf2 = self.findFriends(self.friends)
        ...
        self.friendsOf3 = self.findFriends(self.friendsOf2)
        # Adds friends of friends of friends to network and removes duplicates.

        self.network = self.network + self.friendsOf3     
    ...
    def __repr__(self):
        ...
        outStr += "Friends:\n"
        for count, line in enumerate(self.friends):    
            outStr += (line + "\t")
            if (count+1)%col == 0:
                outStr += "\n"
        outStr += "\nFriends of Friends: \n"
        for count, line in enumerate(self.friendsOf2):    
            outStr += (line + "\t")
            if (count+1)%col == 0:
                outStr += "\n"
        outStr += "\nFriends of Friends of Friends: \n"
        for count, line in enumerate(self.friendsOf3):    
            outStr += (line + "\t")
            if (count+1)%col == 0:
                outStr += "\n"
        return outStr
